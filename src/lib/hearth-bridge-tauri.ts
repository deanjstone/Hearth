...
// each call into a real Tauri `invoke`.
//
// Deliberately partial: Phase 1 (Hearth#27) only ports `selfMod` +
// `frontendReady`, not the full ~90-method/20-namespace `HearthApi` surface
// (see spec #26's "IPC surface" decision — ported per in-scope subsystem as
// it lands, not upfront). The cast below is intentionally unsound today and
// will narrow to a real subset type, or fill in for real, as later phases
// wire their own namespaces through IPC.
//
...
if (typeof window !== 'undefined' && window.__TAURI__ && !window.hearth) {
  window.hearth = { selfMod } as unknown as HearthApi
}
